...
<body class="bg-background dark:bg-background-dark text-on-surface dark:text-on-surface-dark font-sans antialiased min-h-screen flex flex-col transition-colors duration-200">
    
    <!-- Top Navigation Bar -->
    <header class="sticky top-0 z-40 bg-surface/90 dark:bg-surface-dark/90 backdrop-blur-md border-b border-outline-variant/60 dark:border-outline-dark/60">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-1 lg:gap-2">
                <a href="{{ url('/') }}" class="px-3.5 py-2 rounded-lg text-sm font-medium transition-colors {{ request()->is('/') ? 'text-primary dark:text-inverse-primary bg-surface-container dark:bg-surface-container-dark font-semibold' : 'text-on-surface-variant dark:text-on-surface-variant-dark hover:text-primary dark:hover:text-inverse-primary hover:bg-surface-container/50' }}">
                    Beranda
                </a>
                <a href="{{ url('/fleet') }}" class="px-3.5 py-2 rounded-lg text-sm font-medium transition-colors {{ request()->is('fleet*') ? 'text-primary dark:text-inverse-primary bg-surface-container dark:bg-surface-container-dark font-semibold' : 'text-on-surface-variant dark:text-on-surface-variant-dark hover:text-primary dark:hover:text-inverse-primary hover:bg-surface-container/50' }}">
                    Armada Mobil
                </a>
                <a href="{{ url('/admin/cars') }}" class="px-3.5 py-2 rounded-lg text-sm font-medium transition-colors text-on-surface-variant dark:text-on-surface-variant-dark hover:text-primary dark:hover:text-inverse-primary hover:bg-surface-container/50">
            ...
            <div class="flex items-center gap-2.5 sm:gap-3">
                ...
                <div class="hidden sm:flex items-center gap-2">
                    <a href="{{ url('/auth?tab=signin') }}" class="px-4 py-2 text-sm font-semibold text-on-surface-variant dark:text-on-surface-variant-dark hover:text-primary dark:hover:text-inverse-primary transition-colors">
                        Masuk
                    </a>
                    <a href="{{ url('/auth?tab=register') }}" class="px-4 py-2 rounded-lg text-sm font-semibold bg-primary hover:bg-primary-hover text-white shadow-sm shadow-primary/20 transition-all hover:-translate-y-0.5 active:translate-y-0">
                        Daftar Akun
                    </a>
                </div>

                <!-- Mobile Menu Button -->
                <button type="button" onclick="document.getElementById('mobileMenu').classList.toggle('hidden')" class="md:hidden p-2 rounded-lg text-on-surface-variant dark:text-on-surface-variant-dark hover:bg-surface-container dark:hover:bg-surface-container-dark">
        ...
        <div id="mobileMenu" class="hidden md:hidden border-t border-outline-variant/60 dark:border-outline-dark/60 bg-surface dark:bg-surface-dark px-4 pt-3 pb-5 space-y-2">
            <a href="{{ url('/') }}" class="block px-3 py-2 rounded-lg text-base font-medium text-on-surface dark:text-on-surface-dark hover:bg-surface-container dark:hover:bg-surface-container-dark">
                Beranda
            </a>
            <a href="{{ url('/fleet') }}" class="block px-3 py-2 rounded-lg text-base font-medium text-on-surface dark:text-on-surface-dark hover:bg-surface-container dark:hover:bg-surface-container-dark">
                Armada Mobil
            </a>
            <a href="{{ url('/admin/cars') }}" class="block px-3 py-2 rounded-lg text-base font-medium text-primary dark:text-inverse-primary hover:bg-surface-container dark:hover:bg-surface-container-dark">
                Panel Admin Mobil
            </a>
            <div class="pt-3 border-t border-outline-variant/40 dark:border-outline-dark/40 flex flex-col gap-2">
                <a href="{{ url('/auth?tab=signin') }}" class="w-full text-center py-2.5 text-sm font-semibold border border-slate-300 dark:border-slate-700 rounded-lg">
                    Masuk
                </a>
                <a href="{{ url('/auth?tab=register') }}" class="w-full text-center py-2.5 text-sm font-semibold bg-primary text-white rounded-lg">
                    Daftar Akun Baru
                </a>
            </div>
        </div>
    ...
    <!-- Footer -->
    <footer class="bg-white dark:bg-[#071322] border-t border-outline-variant/60 dark:border-outline-dark/60 mt-auto transition-colors">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8 lg:gap-12">
                ...
                <div>
                    ...
                    <ul class="space-y-2.5 text-sm text-text-muted dark:text-text-muted-dark">
                        <li><a href="{{ url('/fleet?type=MPV') }}" class="hover:text-primary dark:hover:text-inverse-primary transition-colors">Keluarga (MPV & Minivan)</a></li>
                        <li><a href="{{ url('/fleet?type=SUV') }}" class="hover:text-primary dark:hover:text-inverse-primary transition-colors">Tangguh (SUV 7-Seater)</a></li>
                        <li><a href="{{ url('/fleet?type=Luxury') }}" class="hover:text-primary dark:hover:text-inverse-primary transition-colors">Eksekutif (Luxury VIP)</a></li>
                        <li><a href="{{ url('/fleet?type=Electric') }}" class="hover:text-primary dark:hover:text-inverse-primary transition-colors">Mobil Listrik (EV Eco)</a></li>
                    </ul>
                </div>
            ...
            <div class="mt-12 pt-6 border-t border-outline-variant/50 dark:border-outline-dark/50 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-text-muted dark:text-text-muted-dark">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <span class="hover:underline cursor-pointer">Syarat & Ketentuan Sewa</span>
                    <span class="hover:underline cursor-pointer">Pusat Bantuan</span>
                </div>
            </div>
        </div>
    </footer>
